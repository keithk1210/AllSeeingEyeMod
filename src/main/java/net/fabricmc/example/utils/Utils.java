...
public class Utils {
    ...
    public static double getAngle(double yaw, double a, double b) {
        ...
        if (degrees > 0 && degrees < 90) {
            ...
            ExampleMod.LOGGER.info("inside fifth condition: a = " + a + ", b = " + b + " yaw = " + yaw + ", degrees = " + degrees);
    ...
    public static int getIdealYawIncrement(double convertedYaw, double targetDiretion) {
        ...
        while (Math.abs(i - targetDiretion ) > 5) {
            //ExampleMod.LOGGER.info("i = " + i + " target direction = " + targetDiretion);
            if (i > 360) {
                i = 0;
            } else {
                i += 1;
        ...
        while (Math.abs(j - targetDiretion) > 5) {
            //ExampleMod.LOGGER.info("j = " + j + " target direction = " + targetDiretion);
            if (j < 0) {
                j = 360;
            } else {
                j -= 1;
